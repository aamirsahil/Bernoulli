    d3.select("#switch").on("input", () => {
        ...
        streamText.attr("x", (switchSate)?widthScale(completeData[7][50].x):widthScale(linearData[7][50].x))
                .attr("y", (switchSate)?heightScale(completeData[7][50].y + 0.5):heightScale(linearData[7][50].y + 0.5));
    ...
    //             let data = x.map( (d) => ({x: d, y: (a<4.5)?f(d, a, sep):a+0.5}));
