...
function cirInNum(value){
    if(value < (sliderEndPos + sliderStartPos)/2){
...
function seTdsText(value){
    if(value == sliderStartPos){
        ...
        d3.selectAll(".ds").data([1,2])
            .attr("visibility", "visible")
            .attr("x", (d) => (d==1)?widthScale(dataVel.slice(0,totalNum)[index].x) + 50
            :widthScale(dataVel.slice(0,totalNum)[index + sectionWidthIndex].x) + 50);
    }
}